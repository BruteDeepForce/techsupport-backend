...
[ApiController]
[Route("api/hr/employee-salaries")]
[Authorize]
public sealed class EmployeeSalariesController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateEmployeeSalaryRequest request, CancellationToken ct)
    {
    ...
    public async Task<IActionResult> GetCurrentByEmployee(Guid employeeId, [FromQuery] DateTime? asOfDate, CancellationToken ct)
    {
        if (!TryGetTenantId(out var tenantId))
        {
            return Unauthorized("TenantId is required in claim (tenant_id).");
        ...
        var result = await _service.GetCurrentByEmployeeAsync(tenantId, employeeId, asOfDate, ct);
        if (!result.Succeeded)
        {
            return result.ErrorType == HRServiceErrorType.NotFound
                ? NotFound(result.Error)
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateEmployeeSalaryRequest request, CancellationToken ct)
    {
        if (!TryGetTenantId(out var tenantId))
        {
            return Unauthorized("TenantId is required in claim (tenant_id).");
