...
[ApiController]
[Route("api/hr/shifts/templates")]
[Authorize]
public sealed class ShiftTemplatesController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateShiftTemplateRequest request, CancellationToken ct)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateShiftTemplateRequest request, CancellationToken ct)
    {
        if (!TryGetTenantId(out var tenantId))
        {
            return Unauthorized("TenantId is required in claim (tenant_id).");
