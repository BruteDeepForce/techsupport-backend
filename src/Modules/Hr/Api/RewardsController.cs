...
public sealed class RewardsController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateRewardRequest request, CancellationToken ct)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateRewardRequest request, CancellationToken ct)
    {
        if (!TryGetTenantId(out var tenantId))
        {
            return Unauthorized("TenantId is required in claim (tenant_id).");
    ...
    [HttpPost("records")]
    public async Task<IActionResult> CreateRecord([FromBody] CreateRewardEmployeeRecordRequest request, CancellationToken ct)
    {
        if (!TryGetTenantId(out var tenantId))
        {
            return Unauthorized("TenantId is required in claim (tenant_id).");
    ...
    public async Task<IActionResult> UpdateRecord(Guid id, [FromBody] UpdateRewardEmployeeRecordRequest request, CancellationToken ct)
    {
        if (!TryGetTenantId(out var tenantId))
        {
            return Unauthorized("TenantId is required in claim (tenant_id).");
