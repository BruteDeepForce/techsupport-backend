...
public sealed class AccountingInboxMessage
{
    ...
    public static AccountingInboxMessage Create<T>(Guid messageId, string consumerName, T response) where T : class => new()
    {
        MessageId = messageId,
    ...
    public (object Response, Type Type) DeserializeResponse()
    {
        ...
        return (JsonSerializer.Deserialize(ResponsePayload, type)
                ?? throw new InvalidOperationException($"Cannot deserialize accounting inbox response {ResponseType}."), type);
    }
}
